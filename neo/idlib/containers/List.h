...
template< class type >
ID_INLINE int32_t idListSortCompare( const type *a, const type *b ) 
{
	assert( 0 );
	return 0;
}
...
ID_INLINE void* idListArrayNew( const uint32_t num, const bool zeroBuffer )
{
	_type_ * ptr = nullptr;
	if( zeroBuffer )
...
ID_INLINE void idListArrayDelete( void* ptr, const uint32_t num )
{
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE void* idListArrayResize( void* voldptr, const uint32_t oldNum, const uint32_t newNum, const bool zeroBuffer )
{
	...
	if( newNum > 0 )
	{
		...
		int32_t overlap = Min( oldNum, newNum );
...
public:
	...
	typedef int32_t		cmp_t( const _type_*, const _type_* );
	typedef _type_	new_t();
	
	idList( const uint32_t newgranularity = 16 );
	...
	const uint32_t	Num( void ) const;
	
	///@brief returns number of elements allocated for
	const uint32_t	NumAllocated( void ) const;
	
	///@brief set new granularity
	void			SetGranularity( uint32_t newgranularity );
	
	///@brief get the current granularity
	const uint32_t	GetGranularity( void ) const;
	
	///@brief returns total size of allocated memory
	const size_t	Allocated( void ) const;
	
	///@brief returns total size of allocated memory including size of list _type_
	...
	_type_& 		operator[]( uint32_t index );
	
	///@brief resizes list to exactly the number of elements it contains
	void			Condense( void );
	
	///@brief resizes list to the given number of elements
	void			Resize( const uint32_t newsize );
	
	///@brief resizes list and sets new granularity
	void			Resize( const uint32_t newsize, const uint32_t newgranularity );
	
	///@brief set number of elements in list and resize to exactly this number if needed
	void			SetNum( const uint32_t newnum );
	
	///@brief assure list has given number of elements, but leave them uninitialized
	void			AssureSize( const uint32_t newSize );
	
	///@brief assure list has given number of elements and initialize any new elements
	void			AssureSize( const uint32_t newSize, const _type_ &initValue );

	///@brief assure the pointer list has the given number of elements and allocate any new elements 
	void			AssureSizeAlloc( const uint32_t newSize, new_t* allocator );
																					
	_type_&			First( void );
	_type_&			Last( void );
	const _type_&	First( void ) const;
	const _type_&	Last( void ) const;

	///@brief returns a pointer to the list
	_type_* 		Ptr( void );
	
	///@brief returns a pointer to the list
	const _type_* 	Ptr( void ) const;										
	
	///@brief returns reference to a new data element at the end of the list
	_type_& 		Alloc( void );											
	
	///@brief append element
	uint32_t		Append( const _type_ & obj );						
	
	///@brief append list
	uint32_t		Append( const idList& other );						
	
	///@brief add unique element
	uint32_t		AddUnique( const _type_ & obj );					
	...
	uint32_t		IndexOf( const _type_ *obj ) const;					
	
	///@brief remove the element at the given index
	bool			RemoveIndex( const uint32_t index );							
	
	///@brief removes the element at the given index and places the last element into its spot - DOES NOT PRESERVE LIST ORDER
	bool			RemoveIndexFast( const uint32_t index );
...
//	void			SortSubSection( int32_t startIndex, int32_t endIndex, cmp_t *compare = ( cmp_t * )&idListSortCompare<_type_> );
	
	///@brief swap the contents of the lists
	void			Swap( idList& other );								
	
	///@brief delete the contents of the list
...
/*
================
idList<_type_,_tag_>::idList( int32_t )
================
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE idList<_type_, _tag_>::idList( const uint32_t newgranularity )
{
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE idList<_type_, _tag_>::~idList( void )
{
	Clear();
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE void idList<_type_, _tag_>::Clear( void )
{
	if( list )
...
ID_INLINE void idList<_type_, _tag_>::DeleteContents( bool clear )
{
	...
	if( clear )
		...
		std::memset( list, 0, size * sizeof( _type_ ) );
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE const size_t idList<_type_, _tag_>::Allocated( void ) const
{
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE const size_t idList<_type_, _tag_>::Size( void ) const
{
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE const size_t idList<_type_, _tag_>::MemoryUsed( void ) const
{
	return num * sizeof( _type_ );
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE const uint32_t idList<_type_, _tag_>::Num( void ) const
{
	return num;
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE const uint32_t idList<_type_, _tag_>::NumAllocated( void ) const
{
	return size;
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE void idList<_type_, _tag_>::SetNum( const uint32_t newnum )
{
	if( newnum > size )
		Resize( newnum );
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE void idList<_type_, _tag_>::SetGranularity( uint32_t newgranularity )
{
	uint32_t newsize = 0;
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE const uint32_t idList<_type_, _tag_>::GetGranularity( void ) const
{
	return granularity;
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE void idList<_type_, _tag_>::Resize( const uint32_t newsize )
{
	assert( newsize >= 0 );
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE void idList<_type_, _tag_>::Resize( const uint32_t newsize, const uint32_t newgranularity )
{
	assert( newgranularity > 0 );
	granularity = newgranularity;
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE void idList<_type_, _tag_>::AssureSize( const uint32_t newSize )
{
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE void idList<_type_, _tag_>::AssureSize( const uint32_t newSize, const _type_ &initValue )
{
	uint32_t newNum = newSize;
	...
	if( newSize > size )
	{
		...
		for( uint32_t i = num; i < newCapacity; i++ )
		{
			list[i] = initValue;
		}
	}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE void idList<_type_, _tag_>::AssureSizeAlloc( const uint32_t newSize, new_t* allocator )
{
	uint32_t newNum = newSize;
	...
	if( newSize > size )
	{
		...
		for( int32_t i = num; i < newCapacity; i++ )
		{
			list[i] = ( *allocator )();
		}
	}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE const _type_& idList<_type_, _tag_>::operator[]( uint32_t index ) const
{
	assert( index < num );	
	return list[ index ];
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE _type_& idList<_type_, _tag_>::operator[]( uint32_t index )
{
	assert( index < num );	
	return list[ index ];
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE _type_& idList<_type_, _tag_>::First( void )
{
	return list[0];
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE _type_& idList<_type_, _tag_>::Last( void )
{
	return list[ Num()-1 ];
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE const _type_& idList<_type_, _tag_>::First( void ) const
{
	return list[ 0 ];
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE const _type_& idList<_type_, _tag_>::Last( void ) const
{
	return list[ Num() - 1 ];
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE _type_* idList<_type_, _tag_>::Ptr( void )
{
	return list;
}
...
*/
template< typename _type_, memTag_t _tag_ >
const ID_INLINE _type_* idList<_type_, _tag_>::Ptr( void ) const
{
	return list;
}
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE _type_& idList<_type_, _tag_>::Alloc( void )
{
	if( !list )
		Resize( granularity );
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE uint32_t idList<_type_, _tag_>::Append( _type_ const& obj )
{
	if( !list )
		Resize( granularity );
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE uint32_t idList<_type_, _tag_>::Insert( _type_ const& obj, uint32_t index )
{
	if( !list )
		Resize( granularity );
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE uint32_t idList<_type_, _tag_>::Append( const idList< _type_, _tag_ >& other )
{
	if( !list )
	{
		if( granularity == 0 )  	// this is a hack to fix our memset classes
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE uint32_t idList<_type_, _tag_>::AddUnique( _type_ const& obj )
{
	int32_t index = 0;
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE int32_t idList<_type_, _tag_>::FindIndex( _type_ const& obj ) const
{	
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE int32_t idList<_type_, _tag_>::FindNull( void ) const
{	
	for( int32_t i = 0; i < num; i++ )
	{
		if( list[ i ] == nullptr )
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE uint32_t idList<_type_, _tag_>::IndexOf( _type_ const* objptr ) const
{
	uint32_t index;
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE bool idList<_type_, _tag_>::RemoveIndex( const uint32_t index )
{	
	assert( list != nullptr );
	assert( index < num );
...
*/
template< typename _type_, memTag_t _tag_ >
ID_INLINE bool idList<_type_, _tag_>::RemoveIndexFast( const uint32_t index )
{
...
ID_INLINE void idList<_type_,_tag_>::Sort( cmp_t *compare ) 
{
	...
	typedef int32_t cmp_c(const void *, const void *);
...
//*/
//template< typename _type_, memTag_t _tag_ >
//ID_INLINE void idList<_type_,_tag_>::SortSubSection( int32_t startIndex, int32_t endIndex, cmp_t *compare ) {
//	if ( !list ) {
//		return;
//	}
//	if ( startIndex < 0 ) {
