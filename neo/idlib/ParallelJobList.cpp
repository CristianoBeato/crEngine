	...
	jobList()
{
	...
	std::memset( &deferredThreadStats, 0, sizeof( threadStats_t ) );
	std::memset( &threadStats, 0, sizeof( threadStats_t ) );
}
...
void idParallelJobList_Threads::Submit( idParallelJobList_Threads* waitForJobList, int parallelism )
{
	...
	std::memset( &deferredThreadStats, 0, sizeof( deferredThreadStats ) );
...
void idParallelJobList_Threads::Wait()
{
	...
	std::memcpy( & threadStats, & deferredThreadStats, sizeof( threadStats ) );
	done = true;
}
