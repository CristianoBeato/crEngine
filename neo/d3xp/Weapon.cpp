...
const idEventDef EV_Weapon_NumProjectiles( "NumProjectiles", nullptr, 'f' );
const idEventDef EV_Weapon_Spread( "Spread", nullptr, 'f' );
...
idWeapon::idWeapon()
{
	...
	worldModel				= nullptr;
	weaponDef				= nullptr;
	weaponThread			= nullptr;
	
	std::memset( &guiLight, 0, sizeof( guiLight ) );
	std::memset( &muzzleFlash, 0, sizeof( muzzleFlash ) );
	std::memset( &worldMuzzleFlash, 0, sizeof( worldMuzzleFlash ) );
	std::memset( &nozzleGlow, 0, sizeof( nozzleGlow ) );
...
void idWeapon::Restore( idRestoreGame* savefile )
{
	...
	for( int i = 0; i < particleCount; i++ )
	{
		...
		std::memset( &newParticle, 0, sizeof( newParticle ) );
	...
	for( int i = 0; i < lightCount; i++ )
	{
		...
		std::memset( &newLight, 0, sizeof( newLight ) );
...
void idWeapon::Clear()
{
	...
	std::memset( &refSound, 0, sizeof( refSound_t ) );
	...
	{	
		...
		for( i = 0; i < num_projectiles; i++ )
		{
			...
			if( i == 0 )
			{
				...
				gameLocal.clip.Translation( tr, muzzle_pos, muzzle_pos + dir * 4096.0f, nullptr, mat3_identity, MASK_SHOT_RENDERMODEL, owner );
				if ( tr.fraction < 1.0f ) {
