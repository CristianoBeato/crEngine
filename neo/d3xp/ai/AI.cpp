...
bool idAASFindAreaOutOfRange::TestArea( const idAAS* aas, int areaNum )
{
	...
	gameLocal.clip.TracePoint( trace, targetPos, areaCenter + idVec3( 0.0f, 0.0f, 1.0f ), MASK_AI_VISION, nullptr );
...
void idAI::Restore( idRestoreGame* savefile )
{
	...
	for( int i = 0; i < emitterCount; i++ )
	{
		...
		std::memset( &newEmitter, 0, sizeof( newEmitter ) );
...
void idAI::SpawnDamageParticles( const char* keyName )
{
	...
	while( kv )
	{
		...
		if( particleName.Length() )
		{
			const idKeyValue* jv = spawnArgs.MatchPrefix( "damageJoint", nullptr );
			while( jv )
			{
