...
idRoutingCache::idRoutingCache( int size )
{
	...
	std::memset( reachabilities, 0, size * sizeof( reachabilities[0] ) );
	travelTimes = new( TAG_AAS ) unsigned short[size];
	std::memset( travelTimes, 0, size * sizeof( travelTimes[0] ) );
}
...
void idAASLocal::UpdateAreaRoutingCache( idRoutingCache* areaCache ) const
{
	...
	std::memset( startAreaTravelTimes, 0, sizeof( startAreaTravelTimes ) );
