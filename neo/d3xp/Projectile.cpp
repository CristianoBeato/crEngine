	...
	launchAxis( mat3_identity )
{
	...
	std::memset( &projectileFlags, 0, sizeof( projectileFlags ) );
	std::memset( &renderLight, 0, sizeof( renderLight ) );
...
void idGuidedProjectile::Launch( const idVec3& start, const idVec3& dir, const idVec3& pushVelocity, const float timeSinceFire, const float launchPower, float dmgPower, const int projNum )
{
	...
	if( owner.GetEntity() )
	{
		...
		*/
			...
			gameLocal.clip.Translation( tr, start, target,  nullptr, mat3_identity, MASK_SHOT_RENDERMODEL, this );
			if ( tr.fraction < 1.0f ) {
				enemy = gameLocal.GetTraceEntity( tr );
...
idBFGProjectile::idBFGProjectile()
{
	std::memset( &secondModel, 0, sizeof( secondModel ) );
...
void idBFGProjectile::Launch( const idVec3& start, const idVec3& dir, const idVec3& pushVelocity, const float timeSinceFire, const float power, const float dmgPower )
{
	...
	for( int e = 0; e < numListedEntities; e++ )
	{
		...
		beamTarget_t bt;
		std::memset( &bt.renderEntity, 0, sizeof( renderEntity_t ) );
