...
uint64_t crTimeQueries::Retrieve( void )
{
    ...
    crVulkanRenderDevicep device = tr.GetRenderDevice();
    // Reset and get results - we should be sure that the GPU is done at this point,
    // for example by waiting on a semaphore
    // VK_QUERY_RESULT_64_BIT to get 64 bit timestamps
    // VK_QUERY_RESULT_WAIT_BIT make the CPU wait for the results ( may remove in release build )
    vkGetQueryPoolResults( *device, m_pools[m_frameID], 0, 2, sizeof(uint64_t) * 2, results, sizeof(uint64_t), VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT );
    vkResetQueryPool( *device, m_pools[m_frameID], 0, 2 );
    m_frameID = ( m_frameID + 1 ) % SMP_FRAMES;
