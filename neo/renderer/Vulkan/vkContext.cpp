...
{    
	...
	idList<const char*>                 requiredInstanceExtensions;
    idList<const char*>                 requiredDeviceExtensions;
    idList<const char*>                 requestedInstanceLayers;
    ...
    common->Printf( "SDL3 VkExt Found:\n" );    
    ...
    window = static_cast<SDL_Window*>( sys->GetVideoSystem()->WindowHandler() );
    if ( !SDL_Vulkan_CreateSurface( window, m_instance, k_allocationCallbacks, &m_surface ) )
    {
        common->Error( "Failed to create window surface %s\n", SDL_GetError() );
        return false;
    }
    ...
    {
        common->Error( "No vulkan suported physical device found." );
        return false;
    }
    ...
    {
        m_devices[i].Init( m_availablePhysicalDevices[i], m_surface ); 
    }
    
    common->Printf( " %i Vulkan suported devices found\n", physicalDeviceCount );
    
    auto FallBack = [physicalDeviceCount, &device]( idList<vkRenderDevice, TAG_VULKAN> &devices, idList<const char*> layers, const idList<const char*> extensions )
    {   
        for ( uint32_t i = 0; i < physicalDeviceCount; i++)
        {
...
/*
==============
vkContext::SurfaceFormat
==============
*/
VkSurfaceFormatKHR vkContext::SurfaceFormat(void) const
{
...
/*
==============
vkContext::PresentMode
==============
*/
VkPresentModeKHR vkContext::PresentMode(void) const
{
