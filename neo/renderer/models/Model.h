...
typedef idList<crDrawGeometry*, TAG_IDLIB_LIST_TRIANGLES> idTriList;

struct modelSurface_t
{
	int							id;
	const idMaterial* 			shader;
	crDrawGeometry* 			geometry;
};
...
public:
	...
	// Allocates memory for crDrawGeometry::verts and crDrawGeometry::indexes
	// The allocated memory is not initialized.
	// crDrawGeometry::numVerts and crDrawGeometry::numIndexes are set to zero.
	virtual crDrawGeometry* 	AllocSurfaceTriangles( int numVerts, int numIndexes ) const = 0;
	
	// Frees surfaces triangles.
	virtual void				FreeSurfaceTriangles( crDrawGeometry* tris ) const = 0;
