...
// BEATO Begin: Keep al stereo cVars toguether 
	...
	for( int i = 0 ; i < 3 ; i++ )
	{
		...
		if( r_fullscreen.GetInteger() <= 0 )
		{
			...
			if ( r_fullscreen.GetInteger() <= -1 )
				fullscreen = VIDEO_MODE_BORDERLESS;
		}
		else
		{
			uint32_t numModes = 0;
		...
		if( fullInit )
		{
			/// We create uniforms and frame buffers 
			bool stereo = ( stereoRender_enable.GetInteger() == STEREO3D_QUAD_BUFFER ) ? true : false;
			uint8_t samples = r_multiSamples.GetInteger();
			if( backend->StartUp( samples, tr.GetWidth(), tr.GetHeight() ) )
				break;
		}
		else
		{
			...
			// ressize framebuffers
			if( backend->SetScreenParms( samples, tr.GetWidth(), tr.GetHeight() ) )
				break; // it worked
		}
		...
		if( i == 0 )
			continue; // same settings, no stereo
...
void GfxInfo_f( const idCmdArgs& args )
{
	...
	if( r_forceScreenWidthCentimeters.GetFloat() )
	{
		common->Printf( "screen size manually forced to %5.1f cm width (%4.1f\" diagonal)\n",
						tr.GetPhysicalScreenWidthInCentimeters(), tr.GetPhysicalScreenWidthInCentimeters() / 2.54f
						* sqrt( ( float )( 16 * 16 + 9 * 9 ) ) / 16.0f );
	}
}
...
void R_VidRestart_f( const idCmdArgs& args )
{
	...
	if( full )
	{
		...
		/// release current render device
		if( tr.m_renderDevice != nullptr )
			tr.m_renderDevice->Destroy();
	...
	else
	{
		...
		/// update framebuffers 
		backend->SetScreenParms( r_multiSamples.GetInteger(), tr.GetWidth(), tr.GetHeight() );
	}
...
{
	/// Initialize Vulkan Logic Render Device
	InitRenderAPI();
...
========================
*/
void idRenderSystemLocal::ResetGuiModels( void )
{
	delete guiModel;
	guiModel = new( TAG_RENDER ) idGuiModel;
	guiModel->Clear();
...
{ 
	static_cast<idImageManagerLocal*>( idImageManager::Get() )->BeginLevelLoad();
...
{
	static_cast<idImageManagerLocal*>( idImageManager::Get() )->LoadLevelImages( false );
}
...
========================
*/
void idRenderSystemLocal::ResetFonts( void )
{
...
void idRenderSystemLocal::InitRenderAPI( void )
{
	...
	if( !IsInitialized() )
	{
		...
		idLib::Printf( "Vulkan Version: %3.1f\n", glVersion );
		idLib::Printf( "Vulkan Vendor : %s\n", glConfig.vendor_string );
		idLib::Printf( "Vulkan GLSL   : %3.1f\n", glslVersion );
#endif
...
========================
*/
uint32_t idRenderSystemLocal::GetHeight( void ) const
{
#ifdef BUGFIXEDSCREENSHOTRESOLUTION
	// foresthale 2014-03-01: screenshots need to override the results of GetWidth() and GetHeight()
...
// BEATO Begin:
void idRenderSystemLocal::UpdateRenderSize( const uint32_t in_width, const uint32_t in_height )
{
	r_windowWidth.SetInteger( in_width );
	r_windowHeight.SetInteger( in_height );
	glConfig.nativeScreenWidth = in_width;
	glConfig.nativeScreenHeight = in_height;
}
// BEATO End