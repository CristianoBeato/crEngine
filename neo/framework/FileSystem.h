...
public:
	...
	/// @brief Restarts the file system.
	virtual void			Restart( void ) = 0;
	
	/// @brief Shutdown the file system.
	virtual void			Shutdown( bool reloading ) = 0;

	/// @brief Returns true if the file system is initialized.
	virtual bool			IsInitialized() const = 0;

	/// @brief Lists files with the given extension in the given directory.
	...
	/// @brief Lists files in the given directory and all subdirectories with the given extension.
	/// Directory should not have either a leading or trailing '/'
	...
	/// @brief Converts a relative path to a full OS path.
	virtual const char* 	OSPathToRelativePath( const char* OSPath ) = 0;

	/// @brief Converts a full OS path to a relative path.
	virtual const char* 	RelativePathToOSPath( const char* relativePath, const char* basePath = "fs_basepath" ) = 0;

	/// @brief Builds a full OS path from the given components.
	...
	/// @brief Creates the given OS path for as far as it doesn't exist already.
	virtual void			CreateOSPath( const char* OSPath ) = 0;

	/// @brief Reads a complete file.
	...
	virtual intptr_t		ReadFile( const char* relativePath, void** buffer, ID_TIME_T* timestamp = nullptr ) = 0;

	/// @brief Frees the memory allocated by ReadFile.
	virtual void			FreeFile( void* buffer ) = 0;

	/// @brief Writes a complete file, will create any needed subdirectories.
	/// @returns the length of the file, or -1 on failure.
	virtual intptr_t		WriteFile( const char* relativePath, const void* buffer, size_t size, const char* basePath = "fs_savepath" ) = 0;
	...
	/// @brief Renames a file, taken from idTech5 (minus the fsPath_t)
	...
	/// @brief Opens a file for reading, reads the file completely in memory and returns an idFile_Memory obj.
	virtual idFile* 		OpenFileReadMemory( const char* relativePath, bool allowCopyFiles = true, const char* gamedir = nullptr ) = 0;

	/// @brief Opens a file for writing, will create any needed subdirectories.
	virtual idFile* 		OpenFileWrite( const char* relativePath, const char* basePath = "fs_savepath" ) = 0;

	/// @brief Opens a file for writing at the end.
	virtual idFile* 		OpenFileAppend( const char* filename, bool sync = false, const char* basePath = "fs_basepath" ) = 0;

	/// @brief Opens a file for reading, writing, or appending depending on the value of mode.
	virtual idFile* 		OpenFileByMode( const char* relativePath, fsMode_t mode ) = 0;

	/// @brief Opens a file for reading from a full OS path.
	virtual idFile* 		OpenExplicitFileRead( const char* OSPath ) = 0;

	/// @brief Opens a file for writing to a full OS path.
	...
	/// @brief look for a dynamic module
	virtual void			FindDLL( const char* basename, char dllPath[ MAX_OSPATH ] ) = 0;
	
	/// @brief don't use for large copies - allocates a single memory block for the copy
	virtual void			CopyFile( const char* fromOSPath, const char* toOSPath ) = 0;
	
	/// @brief look for a file in the loaded paks or the addon paks
	...
	/// @brief ignore case and seperator char distinctions
	virtual bool			FilenameCompare( const char* s1, const char* s2 ) const = 0;
	
	/// @brief This is just handy
	...
	/// @brief Returns length of file, -1 if no file exists
